...
API_ID = int(environ.get('API_ID', '23631217'))  # Telegram API ID
API_HASH = environ.get('API_HASH', '567c6df308dc6901790309499f729d12')  # Telegram API Hash
BOT_TOKEN = environ.get('BOT_TOKEN', '')  # Telegram Bot Token

# 👑, Channels & Logs
BIN_CHANNEL = int(environ.get("BIN_CHANNEL", '-1002338765286'))  # File storage channel
LOG_CHANNEL = int(environ.get("LOG_CHANNEL", '-1002338765286'))  # General log channel
PREMIUM_LOGS = int(environ.get("PREMIUM_LOGS", '-1002399750144'))  # Premium user actions log
VERIFIED_LOG = int(environ.get('VERIFIED_LOG', '-1002399750144'))  # Verified user actions log
SUPPORT_GROUP = int(environ.get("SUPPORT_GROUP", "-1003351490855"))

# add admin IDs 11111 2222 3333 and add auth channel IDs -100XXX -100XXX -100XXX
ADMINS = list(map(int, environ.get('ADMINS', '6139759254').split()))  # List of admin user IDs
AUTH_CHANNEL = list(map(int, environ.get("AUTH_CHANNEL", "-1002232443823 -1002761395296").split()))  # Allowed channels for authorization

# username add without @
OWNER_USERNAME = environ.get("OWNER_USERNAME", 'mimam_officialx')  # Owner's username
BOT_USERNAME = environ.get("BOT_USERNAME", 'MRN_F2LBOT')  # Bot's username

# 🔗 Channel & Support Links
CHANNEL = environ.get('CHANNEL', 'https://t.me/Mrn_Officialx')  # Updates channel
SUPPORT = environ.get('SUPPORT', 'https://t.me/+lTO1v9u3v4FhODNl')  # Support group
HOW_TO_VERIFY = environ.get('HOW_TO_VERIFY', 'https://t.me/MRN_Tutorial/1814')  # Verification guide link
HOW_TO_OPEN = environ.get('HOW_TO_OPEN', 'https://t.me/MRN_Tutorial/1814')  # File access guide link
...
# 📸 all Media (Images)
QR_CODE = environ.get('QR_CODE', 'https://files.catbox.moe/5ns27j.jpg')  # QR Code image
VERIFY_IMG = environ.get("VERIFY_IMG", "https://files.catbox.moe/7udonz.jpg")  # Verify success image
AUTH_PICS = environ.get('AUTH_PICS', 'https://files.catbox.moe/zw41yz.jpg')  # Auth step image
PICS = environ.get('PICS', 'https://files.catbox.moe/cplv64.jpg')  # Default info image
FILE_PIC = environ.get('FILE_PIC', 'https://files.catbox.moe/cplv64.jpg') # file image 
...
MAX_FILES = int(environ.get("MAX_FILES", "100"))  # Max files allowed per user
...
name = str(environ.get('name', 'mzbotz'))  # Project name
APP_NAME = None
if 'DYNO' in environ:
    ON_HEROKU = True
...
FQDN = getenv("FQDN", "https://mrn-universe-stream-bot.koyeb.app/") or BIND_ADDRESS  # Full domain name or fallback to bind address
PORT_SEGMENT = "" if NO_PORT else f":{PORT}/"  # Port in URL if not disabled
PROTOCOL = "https://mrn-universe-stream-bot.koyeb.app/" if HAS_SSL else "https://mrn-universe-stream-bot.koyeb.app/"  # Protocol for URL
URL = f"https://mrn-universe-stream-bot.koyeb.app/"  # Final generated base URL
